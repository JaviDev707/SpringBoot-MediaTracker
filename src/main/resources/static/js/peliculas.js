...
// ========================== Carga ============================ 
/**
 * Funcion que carga todas los peliculas en la tabla
 */
function cargarPeliculas() {
    fetch("http://localhost:8080/pelicula/todos")
        .then(response => response.json())
        .then(peliculas => cargarTabla(peliculas))
        .catch(error => console.error("Error al cargar peliculas:", error));
}
/**
 * Funcion que carga peliculas en la tabla
 */
function cargarTabla(peliculas){
                ...
                fila.innerHTML = `
                    <td>
                      <div class="d-flex align-items-center">
                        <img src="https://png.pngtree.com/element_pic/16/11/18/4f829e85866bd52d062ca58b4e1ecef5.png" 
                             class="avatar sm rounded-pill me-3 flex-shrink-0" alt="pelicula">
                        <div class="h6 mb-0 lh-1">${pelicula.titulo}</div>
                      </div>
                    </td>
                    <td>${pelicula.genero}</td>
                    <td>${pelicula.plataforma}</td>
                    <td>${pelicula.año}</td>
                    <td>${pelicula.finalizacion}</td>
                    <td>
                    ...
                    <td class="text-end">
                        <button class="btn btn-primary btn-sm" onclick="activarEdicion(this, ${pelicula.id})"><i class="bi bi-pencil"></i></button>
                        <button class="btn btn-danger btn-sm" onclick="borrarPelicula(${pelicula.id})"><i class="bi bi-trash"></i></button>
                    </td>
                `;

                tbody.appendChild(fila);
                activarRating(fila, pelicula.rating);
            });
}
...
function guardarRegistro() {
    ...
    fetch("http://localhost:8080/pelicula/registro", {
        method: "POST",
        headers: {
    ...
    .then(response => {
        if (response.ok) {
            ...
            alert("Error al registrar la pelicula");
        }
    })
...
function guardarEdicion(boton, id) {
    ...
    fetch(`http://localhost:8080/pelicula/actualizar/${id}`, {
        method: "PATCH",
        headers: {
            "Content-Type": "application/json"
        },
...
// ========================== Borrar ============================
/**
 * Funcion que borra una pelicula enviando su id y llamando a http://localhost:8080/pelicula/borrar/${id}
 */
function borrarPelicula(id) {
    if (!confirm("¿Estás seguro de que quieres borrar esta pelicula?")) {
        return;
    }

    fetch(`http://localhost:8080/pelicula/borrar/${id}`, {
        method: "DELETE"
    })
    .then(response => {
...
document.getElementById("btnFiltrar").addEventListener("click", function() {
    ...
    if (valor === "") {
        cargarPeliculas(); // Mostrar todo si no escribe nada
        return;
    }
...
function filtrarPorTitulo(titulo){
    fetch(`http://localhost:8080/pelicula/titulo/${titulo}`)
        .then(response => response.json())
        .then(peliculas => cargarTabla(peliculas))
        .catch(error => console.error("Error al cargar peliculas:", error));
}
...
function filtrarPorPlataforma(plataforma){
    fetch(`http://localhost:8080/pelicula/plataforma/${plataforma}`)
        .then(response => response.json())
        .then(peliculas => cargarTabla(peliculas))
        .catch(error => console.error("Error al cargar peliculas:", error));
}
...
function filtrarPorGenero(genero){
    fetch(`http://localhost:8080/pelicula/genero/${genero}`)
        .then(response => response.json())
        .then(peliculas => cargarTabla(peliculas))
        .catch(error => console.error("Error al cargar peliculas:", error));
}
...
function filtrarPorMes(valor){
    ...
    fetch(`http://localhost:8080/pelicula/mes_año/${mes}/${año}`)
        .then(response => response.json())
        .then(peliculas => cargarTabla(peliculas))
        .catch(error => console.error("Error al cargar peliculas:", error));
}
...
function filtrarPorAño(año){
    fetch(`http://localhost:8080/pelicula/año/${año}`)
        .then(response => response.json())
        .then(peliculas => cargarTabla(peliculas))
        .catch(error => console.error("Error al cargar peliculas:", error));
}
